...
export const universeIntegrationActions: ActionTree<UniverseIntegrationState, RootState> = {
  async FETCH_INTEGRATION({ state, commit, rootState }) {
    ...
    try {
      ...
      let filterString = encodeURI(`app=universe and provider=universe`);
      // @ts-ignore
  ...
  async FETCH_TASKS({ state, commit, rootState }, { oid, top, orderby }) {
    ...
    try {
      ...
      url += `&$filter=${encodeURIComponent(`promoterIntegrationOid=${oid} AND name="event-data-synch"`)}`;
