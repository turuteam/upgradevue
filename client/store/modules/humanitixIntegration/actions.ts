...
export const humanitixIntegrationActions: ActionTree<HumanitixIntegrationState, RootState> = {
  async CREATE_HUMANITIX_INTEGRATION({ commit }, auth: { apiKey: string, accountName: string}) {
    try {
      commit('SET_IS_CREATING_INTEGRATION', true);
      const syncTask = await this.$api.promoterIntegrations.humanitixSimpleAuth(auth.apiKey, auth.accountName);
      ...
      // override service messages with something more meaningful to Humanitix integrations
      let customMessages: {[key: number]: string} = {
        409: `An integration already exists for the provided Humanitix account`,
        // ...
      };

      this.$arNotification.push({
        type: 'error',
        message: customMessages[serviceStatus] || serviceMessage || 'An error occurred connecting to Humanitix'
      });
      return false;
    } finally {
  ...
  async FETCH_INTEGRATION({ commit, rootState }) {
    ...
    try {
      ...
      const integrations = await this.$api.promoterIntegrations.fetchByAppProvider(promoterOid, 'humanitix', 'humanitix');
